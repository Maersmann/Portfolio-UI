...
{
    public class ZinsenVergleichMonatViewModel : ViewModelAuswertung<ZinsenVergleichMonatModel>
    {
        private int jahrvon;
        private int jahrbis;
        private ZinsenBetragTyp typ;

        public ZinsenVergleichMonatViewModel()
        {
            Title = "Auswertung Zinsen je Monat - Jahresvergleich";
        ...
        private async void ExcecuteLoadDataCommand()
        {
            ...
            HttpResponseMessage resp = await Client.GetAsync(GlobalVariables.BackendServer_URL + $"/api/auswertung/zinsen/Gesamt/Monat/Vergleich?jahrVon={jahrvon}&jahrBis={jahrbis}");
            if (resp.IsSuccessStatusCode)
            {
                ItemList = await resp.Content.ReadAsAsync<List<ZinsenVergleichMonatModel>>();
                SetDataIntoChart();
        ...
        private void SetDataIntoChart()
        {
            ...
            ColumnSeries<decimal>[] series = new ColumnSeries<decimal>[ItemList.Count];
            int index = 0;
            ItemList.ToList().ForEach(item =>
            {
                ColumnSeries<decimal> coloumn = new ColumnSeries<decimal>
                {
